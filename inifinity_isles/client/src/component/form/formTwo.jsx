...
import { updateProduct } from "../../../apps/action/prodAction";

const FormTwo = ({item}) => {
    const {category, productId} = useParams()
    const [text, setText] = useState(item?.description || "");
    const [url, setUrl] = useState(item?.images || '');
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        dispatch(updateProduct(productId, fields.title, fields.description, fields.stock, fields.price, item.category, url))
            ...
            <div className="flex justify-between w-11/12 mx-auto">
                <form action="" className="inline-flex flex-col justify-center items-center my-3 mx-5" onSubmit={handleSubmit}>
                    ...
                    <div className="flex justify-between w-full px-1">
                        <button type="submit" className={` text-white bg-blue-600 ${!emptyField() ? "cursor-pointer" : "cursor-not-allowed opacity-25"} rounded-md py-1 px-5`}>Update Product</button>
                        <button className="block text-blue-600 font-medium border-2 border-blue-600 rounded-md px-5" onClick={handleCancel}>Cancel</button>
                    </div>
                </form>
